...
def processFrame(frame, thresholding):
...
def getHoughLines(frame):
    ...
    if horizontal is not None and vertical is not None:
        ...
        coordsHorizontal = addLines(frame, horizontal, (255, 0, 0))

        vertical = trimLines(vertical)
        vertical = sorted(list(vertical), key=lambda vx: vx[0])
        coordsVerticle = addLines(frame, vertical, (0, 0, 255))
    # cv2.imwrite('testHough.jpg', perspective_frame)
...
def borderCalculator(frame):
    ...
    cv2.imwrite('CORNER_MAPPER.jpeg', frame)
...
def getPiecesFromImage(frame):
    ...
    for ac in approxCorners:
        ...
        if boardTL is None or dist(boardTL, topLeft) > dist(ac, topLeft):
            boardTL = ac
    print(boardBR, boardBL, boardTR, boardTL)
    cornersTrimmed = np.asarray([boardBL, boardTL, boardTR, boardBR])
    for ac in cornersTrimmed:
        cv2.circle(frame, ac, 12, (255, 255, 255), thickness=3)
    # pts_dst = np.array([[3024, 0.0], [3024, 4032], [0, 4032], [0.0, 0.0]])
    cv2.imwrite('testCorners.jpg', frame)
    print(frame.shape[0], frame.shape[1])
    pts_dst = np.array([[0.0, 0.0], [frame.shape[1], 0.0], [frame.shape[1], frame.shape[0]], [0, frame.shape[0]]])  # FOR NO ROTATION

    # Warps Perspective and Resizes to (640, 640)
    perspective_frame = getBoardPerspective(frame, cornersTrimmed, pts_dst)
    cv2.imwrite('testWarp.jpg', perspective_frame)
    perspective_frame = cv2.resize(perspective_frame, (640, 640), interpolation=cv2.INTER_AREA)
    ...
    #         cv2.circle(perspective_frame, centerCoord, 4, (255, 255, 255), thickness=2)
    #     # print('Writing image ' + filename)
    # # cv2.imwrite('./board-coords.jpeg', perspective_frame)
...
def processAndSaveImage():
    ...
    # fp = 'C:/Users/Carter/Desktop/Classes/Chess-Reader/workspace/Backend/img_uploads/board.jpeg'
    frame = cv2.imread(fp)
