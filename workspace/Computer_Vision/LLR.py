...
def LLR(img, points, lines):
    ...
    def __v(l):
        ...
        s1 = llr_polyscore(na(poly1), points, centroid, beta=int(beta), alfa=int(alfa / 2))
        poly2 = llr_polysort([a, b,
                              [img.shape[1], 0], [img.shape[1], img.shape[0]]])
        s2 = llr_polyscore(na(poly2), points, centroid, beta=int(beta), alfa=int(alfa / 2))
    ...
    from workspace.Computer_Vision.LAPS import laps_intersections
    ...
    for v in itertools.combinations(pregroup[0], 2):
        for h in itertools.combinations(pregroup[1], 2):
            ...
            S[-llr_polyscore(poly, points, centroid,
                             beta=int(beta), alfa=int(alfa / 2))] = poly
